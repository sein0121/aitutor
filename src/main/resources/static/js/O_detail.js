...
var teacherNm = getParameter("teacher_nm");
...
//페이지 진입했을 때 API 호출
function onLoadData() {
    API.admin.odetail({data: loadData}, function(result) {
        ...
        //클래스 ID
        if(classId == resultList.class_id) document.getElementById("class_nm").innerText = resultList.class_id; //classId
        ...
        pro_excel.setAttribute("id",teacherNm+"_"+resultList.class_nm);
        pro_excel.setAttribute("onclick","proClick(this.id)");

        //테스트 완료 학생 엑셀 다운로드
        let com_excel = document.getElementById("com_excel");
        com_excel.setAttribute("id",teacherNm+"_"+resultList.class_nm);
...
function drowProTabel(pro_userList){
    ...
    //사용자ID 작성
    pro_tr.innerHTML = "<td style=\'width: 30%;\' id=\'"+pro_userList.user_id+"\'><a href='/aitutor/admin/student/O_sdetail?user_id="+pro_userList.user_id+"&user_grd="+pro_userList.user_grd+"' >"+pro_userList.user_id+"</a></td>";
...
function drowComTabel(com_userList){
    ...
    com_tr.innerHTML = "<td style=\'width: 20%;\' id=\'"+com_userList.user_id+"\'><a href='/aitutor/admin/student/O_sdetail?user_id="+com_userList.user_id+"&user_grd="+com_userList.user_grd+"' >"+com_userList.user_id+"</a></td>";
...
const proValue = (target) => {
    ...
    for(let i=0; i<proList.length;i++){
        ...
        pro_tr.innerHTML = "<td style=\'width: 30%;\'><a href='/aitutor/admin/student/O_sdetail?user_id="+proList[i].user_id+"&user_grd="+proList[i].user_grd+"' >"+proList[i].user_id+"</a></td>";
...
const comValue = (target) => {
    ...
    for(let i=0; i<comList.length;i++){
        ...
        com_tr.innerHTML = "<td style=\'width: 20%;\'><a href='/aitutor/admin/student/O_sdetail?user_id="+comList[i].user_id+"&user_grd="+comList[i].user_grd+"' >"+comList[i].user_id+"</a></td>";
