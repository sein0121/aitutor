...
function windowOnLoad() {
    if(pageType === "class") {
        if(searchType !== "") {
            ...
            // [TU-CL-01] 교사 클래스 목록 페이지 진입 API
            API.tutor.classList({data: loadData}, function(result) {
                console.log('TU-CL-01 CALLED   ', result);
        ...
        if(searchType !== "") {
            ...
            // [TU-ST-01] 교사 학생 목록 페이지 진입 API
            API.tutor.studentList({data: loadData}, function(result) {
                console.log('TU-ST-01 CALLED   ', result);
                // API 결과
...
function render(list, type) {
    ...
    let renderTable = function(page) {
        ...
        let html = '';
        if(list.length === 0) {
            html = '<tr><td colspan="'+ col +'">데이터가 존재하지 않습니다</td> </tr>';
        } else {
            for(let index = startNum; index < endNum; index++) {
                if (type === "class") {
                    ...
                    html += '<tr id="ctr" tr_value="' + list[index].class_id + '"><td><input type="checkbox" id="' + list[index].class_id + '" name="' + list[index].class_id + '"></td>' +
                        ...
                        '<td><a href="/aitutor/teacher/class/T_detail?class_id=' + list[index].class_nm + '">' + list[index].class_nm + '</a></td>' +
                    ...
                    html += '<tr id="str" tr_value="' + list[index].student_id + '"><td><input type="checkbox" id="' + list[index].student_id + '" name="' + list[index].student_id + '"></td>' +
                        ...
                        '<td><a href="/aitutor/teacher/student/T_sdetail?user_id=' + list[index].student_id + '&user_grd=' + list[index].user_grd + '">' + list[index].student_id + '</a></td>' +
...
function del(classId) {
    ...
    document.getElementById("pass_submit").onclick = function() {
        ...
        if(pass === "") {
            ...
            // [TU-CL-05] 교사 클래스 삭제 - 비밀번호 확인 data
            let passData = {
                org_id: orgId,
                user_id: userId,
                user_pw: pass
            };
            // [TU-CL-05] 교사 클래스 삭제 - 비밀번호 확인 API
            API.tutor.password({data: passData}, function(result) {
                console.log('TU-CL-05 CALLED   ', result);
                // API 결과
                if(result.rsp_code === 210) {
                    ...
                    delButton.onclick = function() {
                        ...
                        // [TU-CL-06] 교사 클래스 삭제 data
                        const submitClass = {
                            org_id: orgId,
                            user_id: userId,
                            class_id: classId
                        };
                        // [TU-CL-06] 교사 클래스 삭제 API
                        API.tutor.classDelete({data: submitClass}, function(result) {
                            console.log('TU-CL-06 CALLED   ', result);

                            // [TU-CL-06] 교사 클래스 삭제 API 성공 시 [TU-CL-01] 교사 클래스 목록 페이지 진입 API 호출
...
function delStudent(studentId) {
    ...
    document.getElementById("stud_pass_submit").onclick = function() {
        ...
        if(sPass === "") {
            ...
            // [TU-CL-05] 교사 클래스 삭제 - 비밀번호 확인 API
            API.tutor.password({data: passDatas}, function(result) {
                // API 결과
                if(result.rsp_code === 210) {
                    console.log("일치");
                    ...
                    delButtons.onclick = function() {
                        ...
                        // [TU-ST-06] 교사 학생 목록 학생 삭제 API
                        API.tutor.studentDelete({data: submitStudent}, function(result) {
                            console.log('TU-ST-06 CALLED   ', result);
                            if(result.rsp_code === 200) {
                                // [TU-ST-01] 교사 학생 목록 페이지 진입 API
                                API.tutor.studentList({data: loadData}, function(result) {
                                    console.log('TU-ST-01 CALLED   ', result);
...
document.getElementById("btn_new").onclick = function() {
    ...
    document.getElementById("duplicate").onclick = function() {
        ...
        if(cName === "") {
            ...
            // [TU-CL-03] 교사 클래스 등록 - 이름 중복 확인 data
            let nameData = {
                org_id: orgId,
                class_nm: cName
            };
            // [TU-CL-03] 교사 클래스 등록 - 이름 중복 확인 API
            API.tutor.classname({data: nameData}, function(result) {
                console.log('TU-CL-03 CALLED   ', result);
                // API 결과
                if(result.rsp_code === 220) {
                    ...
                    newButton.onclick = function() {
                        ...
                        // [TU-CL-04] 교사 클래스 등록 API
                        API.tutor.classInsert({data: submitClass}, function(result) {
                            console.log('TU-CL-04 CALLED   ', result);

                            // [TU-CL-04] 교사 클래스 등록 API 성공 시 [TU-CL-01] 교사 클래스 목록 페이지 진입 API 호출
                            if(result.rsp_code === 200) {
                                cName = "";
                                // [TU-CL-01] 교사 클래스 목록 페이지 진입 API
                                API.tutor.classList({data: loadData}, function(result) {
    ...
    if(iList === 0) {
        ...
        if(iList[0].id === '아이디가 존재하지 않습니다.') {
            ...
            if (dup === 0) {
                ...
                API.tutor.overlap({data: overlapData}, function (result) {
                    console.log('TU-ST-05 CALLED   ', result);
                    // API 결과
                    const overList = result.result.student_list;
...
document.getElementById("id-auto").onclick = function () {
    ...
    // [TU-ST-04] 교사 학생 목록 아이디 자동 생성 data
    const autoData = {
        org_id: orgId,
        user_id: userId,
        class_id: className
    };
    // [TU-ST-04] 교사 학생 목록 아이디 자동 생성 API
    API.tutor.auto({data: autoData}, function(result) {
        console.log('TU-ST-04 CALLED   ', result);
...
function classRender() {
    // [TU-ST-03] 교사 학생 목록 클래스정보 조회 data
    const classInfo = {
        org_id: orgId,
        user_id: userId
    };
    // [TU-ST-03] 교사 학생 목록 클래스정보 조회 API
    API.tutor.classInfo({data: classInfo}, function (result) {
        console.log('TU-ST-03 CALLED   ', result);
        // API 결과
...
function searchClass(searchTypes, searchValues) {
    ...
    // [TU-CL-02] 교사 클래스 검색 API
    API.tutor.classSearch({data: searchData}, function(result) {
        console.log('TU-CL-02 CALLED   ', result);
...
function searchStudent(searchTypes, searchValues) {
    ...
    API.tutor.studentSearch({data: searchDatas}, function(result) {
        console.log('TU-ST-02 CALLED   ', result);
        // API 결과
