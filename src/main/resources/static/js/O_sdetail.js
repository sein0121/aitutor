...
// 페이지 로딩 시 API 호출
function onLoadData() {
    API.admin.osdetail({data: loadData}, function (result) {
        // API 호풀 결과
        const loadResult = result;
        console.log("osdetail API 호출",loadResult);
        ...
        if(loadResult.rsp_code==200){
            ...
            for(let i=0; i<test_user_list.length; i++) createDiv(test_user_list[i]);
        }
...
function createDiv(test_list){
    ...
    else document.getElementById("last_score"+level).innerHTML = "테스트<br>진행중";
}
...
function checkedButton(id){
    ...
    if(btnId=="" || btnId != id) answer.style.right = "0px"; //답안지 style
    else if(btnId == id){
...
function viewTest(id){
    ...
    document.getElementById('test-img').setAttribute("src",API.image.viewer(select_test.qst_img_file_nm)); //서버
